...
export const useAdminProfile = (): UseAdminProfileReturn => {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      ...
      if (error) {
        if (import.meta.env.DEV) console.error('Erreur fetch admin_profile:', error);
        setProfile(null);
      } else {
        setProfile({
          ...data,
  ...
  const updateProfile = async (updates: Partial<AdminProfile>): Promise<boolean> => {
    ...
    if (error) {
      if (import.meta.env.DEV) console.error('Erreur update admin_profile:', error);
      return false;
    }
